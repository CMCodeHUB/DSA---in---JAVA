# DSA---in---JAVA

# 🚀 LeetCode 100 DSA Problems in Java

A structured collection of 100 LeetCode DSA problems solved in Java, designed to strengthen problem-solving skills, master core data structures & algorithms, and prepare for technical interviews.

# 📌 About This Repository

This repository contains 100 carefully selected LeetCode problems with solutions written in Java.

The goal is simple:

Learn → Solve → Understand → Practice → Improve

Each problem focuses on an important Data Structures & Algorithms (DSA) concept commonly asked in coding interviews and competitive programming.

This repository is part of my journey to become stronger in DSA, Java, problem solving, and technical interviews.

# 🎯 Goals
🧠 Improve problem-solving and logical thinking
💻 Master Data Structures & Algorithms
☕ Practice DSA using Java
🎯 Solve LeetCode problems consistently
📚 Build strong interview preparation
🔍 Understand multiple approaches to problems
⏱️ Analyze Time & Space Complexity
🚀 Build a strong GitHub DSA portfolio


# 🧩 Topics Covered

This repository covers the most important DSA concepts:

🔢 Arrays
🔤 Strings
🔗 Linked Lists
📚 Stack
🚶 Queue
🌳 Binary Trees
🌲 Binary Search Trees
🔍 Binary Search
🗺️ HashMap & HashSet
🔄 Recursion
🧮 Dynamic Programming
🕸️ Graphs
⚡ Greedy Algorithms
🔀 Sorting
🔎 Searching
🪟 Sliding Window
👆 Two Pointers
📦 Heap / Priority Queue
🌐 Graph Traversal
🌳 Trie
🧠 Backtracking
🧮 Bit Manipulation

# 🧠 DSA Learning Roadmap

The problems are intended to be solved progressively.

Arrays
   ↓
Strings
   ↓
Hashing
   ↓
Two Pointers
   ↓
Sliding Window
   ↓
Stack & Queue
   ↓
Linked List
   ↓
Binary Search
   ↓
Trees
   ↓
Heap
   ↓
Recursion
   ↓
Backtracking
   ↓
Graphs
   ↓
Greedy
   ↓
Dynamic Programming



# 🚀 My DSA Journey

This repository is a record of my continuous DSA practice.

I am solving problems consistently to improve my:

Problem-solving ability
Algorithmic thinking
Java programming
Coding speed
Interview readiness

Consistency beats intensity.

One problem at a time.
One concept at a time.
One step closer to becoming a better problem solver. 🚀


# 👨‍💻 Author

Chandan Mahanta

Student | Java Developer | DSA Learner

Connect with me
GitHub: (https://github.com/CMCodeHUB/)
LinkedIn: Chandan Mahanta


# ⭐ Keep Learning. Keep Solving. Keep Growing.
100 Problems
     ↓
Strong Fundamentals
     ↓
Better Problem Solving
     ↓
Better Coding Skills
     ↓
Interview Ready 🚀

Happy Coding! ☕💻
